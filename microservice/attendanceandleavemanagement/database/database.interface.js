...
exports.insertListDataInDB = function (key, data, res, dbIndex) {
    ...
    db.exists(key, function(err, reply) {
        ...
        if (reply === 1) {
            db.lpush(key, user, function(err, ok){
                if(err){
...
exports.getListDataFromDB = function (key, res, dbIndex, type='def') {
    let db = DatabaseConnection();
    connectToDB(dbIndex);
    db.exists(key, function(err, reply) {
        if(err){
                ...
                if(data) {
                    if(type === 'date') {
                        ...
                        for(let day of data) {
                            ...
                            if(dayObj.leaveDay === true || dayObj.leaveDay === 'true') {
                                days.push(dayObj);
                            }
                        }
                    ...
                    if(type === 'time') {
                        ...
                        for(let time of data) {
                            ...
                            if(timeObj.leaveDay === false || timeObj.leaveDay === 'false') {
                                times.push(timeObj);
                            }
                        }
                        sendResponse(res, statusCodes.SUCCESS, messages.GENERAL_SUCCESS,times);
                        return;
                    }
                    sendResponse(res, statusCodes.SUCCESS, messages.GENERAL_SUCCESS, JSON.parse(data));
                    return;
                }
            });
        }
